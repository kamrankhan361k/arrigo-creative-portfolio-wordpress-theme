...
class Arr_Add_Custom_Fonts {
	...
	public function init_hooks() {
		...
		add_filter( 'arr/kirki_font_choices', array( $this, 'add_custom_fonts' ) );
	}
	...
	public function get_custom_fonts() {
		...
		update_option( 'arr-custom-fonts', Bsf_Custom_Fonts_Taxonomy::get_fonts() );
	}
	...
	public function prepare_custom_fonts_acf() {
		...
		if ( arr_have_rows( 'custom_fonts', 'option' ) ) {
			
			while ( arr_have_rows( 'custom_fonts', 'option' ) ) {
				...
				if ( ! empty( $font_name ) && arr_have_rows( 'font_files', 'option' ) ) {
					while ( arr_have_rows( 'font_files', 'option' ) ) {
	...
	public function prepare_custom_fonts() {
		...
		$fonts = get_option( 'arr-custom-fonts' );
	...
	public function add_custom_fonts( $custom_choice ) {
		...
		$custom_choice['families']['custom'] = array(
			'text'     => esc_attr__( 'Custom Fonts', 'arrigo' ),
	...
	public function print_custom_css() {
		...
		if ( ! empty (self::$css_to_print) ) {
			add_action('wp_enqueue_scripts', function() {
				wp_add_inline_style( 'arrigo-main-style', trim( self::$css_to_print ) );
			}, 30);
		}
